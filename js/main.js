...
MyApp = {
  ...
  validateHome: {
    init: function () {
      ...
      checkbox2.addEventListener('change', function () {
        if (this.checked) {
          label2.classList.add('activo');
        } else {
          label2.classList.remove('activo');
        }
      });
      */
    ...
    init: function () {
      ...
      function checkResolution() {
        ...
        if (window.matchMedia("(min-width: 1025px)").matches) {
          ...
          contentImg.forEach(function (img, i) {
            var waveFooter = gsap.timeline({
              scrollTrigger: {
                //markers: true,
                trigger: img,
                ...
                scrub: true,
                onEnter: function () {
                  //gsap.to(contentText[i], { x: 0 });
                }
              }
            });
            waveFooter.to(contentText[i - 1], { opacity: 0, });
            waveFooter.to(contentText[i], { opacity: 1, });

            
            waveFooter.to(contentText[i-1], { xPercent: 100, opacity: 0, });
            waveFooter.to(contentText[i], { xPercent: -100, });
  ...
  imgFollow: {
    init: function () {
      /*
      const homeSection = document.querySelector('section.home .background-video');
      homeSection.addEventListener('mouseenter', () => {
        homeSection.classList.add('hovered');
      });

      homeSection.addEventListener('mouseleave', () => {
        homeSection.classList.remove('hovered');
      });

      document.body.addEventListener('mousemove', e => {
        const isHovered = homeSection.classList.contains('hovered');

        if (isHovered) {
          document.querySelector('.iconoFollow').classList.remove('quieto');
          document.querySelector('.iconoFollow').classList.add('seguir');
          gsap.to('.iconoFollow', {
            x: e.clientX,
            y: e.clientY,
            stagger: -0.05,
          ...
          document.querySelector('.iconoFollow').classList.add('quieto');
          document.querySelector('.iconoFollow').classList.remove('seguir');
        }
      });
      */
